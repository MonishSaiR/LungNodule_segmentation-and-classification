            ...
            if len(nodules_annotation) > 0:
                ...
                for slice in range(vol.shape[2]):
                    ...
                    try:
                        ...
                        # Save lung segmented array
                        try:
                            np.save(patient_clean_dir_image / nodule_name, lung_segmented_np_array)
                        except FileNotFoundError:
                            # Handle the FileNotFoundError here (or any other specific exception)
                            print("Lung segmented array not saved due to missing directory/file.")
                            continue

                        # Save lung mask array
                        try:
                            np.save(patient_clean_dir_mask / mask_name, lung_mask)
                        except FileNotFoundError:
                            # Handle the FileNotFoundError here (or any other specific exception)
                            print("Lung mask array not saved due to missing directory/file.")
                            continue
